...
export function ProfileClient() {
  ...
  const saveHistory = async (history: AcademicHistoryEntry[]) => {
    const res = await fetch('/api/profile', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  return (
    <div className="max-w-2xl space-y-6">
          ...
          <dl className="space-y-4">
            {[
              ...
              <div key={label} className="flex gap-4">
                <dt className="w-32 shrink-0 text-sm text-gray-500 dark:text-slate-400">{label}</dt>
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-base font-semibold text-gray-900 dark:text-white">Academic History</h3>
        ...
        {!profile?.academicHistory?.length ? (
          <div className="text-center py-8">
            <svg className="mx-auto h-10 w-10 text-gray-300 dark:text-slate-600 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
            </svg>
            <p className="text-sm text-gray-400 dark:text-slate-500">No academic history yet.</p>
            <button onClick={openAddEntry} className="mt-2 text-sm text-indigo-500 hover:underline">Add your first entry</button>
          </div>
        ) : (
            ...
            {profile.academicHistory.map((entry, i) => (
              <li key={i} className="ml-6">
                ...
                <span className="absolute -left-3 flex h-6 w-6 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-900/40 ring-4 ring-white dark:ring-slate-800">
                  <svg className="h-3 w-3 text-indigo-600 dark:text-indigo-400" fill="currentColor" viewBox="0 0 8 8">
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-white">{entry.title}</h4>
                    {entry.description && (
                      <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">{entry.description}</p>
                    )}
                  </div>
                  <div className="flex gap-1 shrink-0">
                    ...
                    <button
                      onClick={() => { if (confirm('Remove this entry?')) deleteEntry(i) }}
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-800 shadow-2xl p-6">
              ...
              <form onSubmit={tlHandleSubmit(onTimelineSubmit)} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Year *</label>
                    <input
                      {...tlRegister('year', { required: true, pattern: /^\d{4}$/ })}
                      className="input w-full"
                      placeholder="2022"
                      maxLength={4}
                    />
                    {tlErrors.year && <p className="text-xs text-red-500 mt-1">Enter a 4-digit year</p>}
                  </div>
                  <div>
                    ...
                    />
                    {tlErrors.title && <p className="text-xs text-red-500 mt-1">Required</p>}
                  </div>
                </div>
                <div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <Button variant="outline" type="button" onClick={() => setTimelineModalOpen(false)}>Cancel</Button>
