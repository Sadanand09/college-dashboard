...
export function AnnouncementsClient() {
  ...
  const deleteAnnouncement = async (id: string) => {
    try {
      ...
      if (res.ok) { toast('Deleted', 'success'); fetchAnnouncements() }
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-300 dark:border-slate-600 py-20 text-center">
          ...
          <p className="text-gray-400 text-sm mb-4">
            {categoryFilter === 'all' ? 'No announcements yet.' : `No ${categoryFilter} announcements.`}
          </p>
          {categoryFilter === 'all' && <Button size="sm" onClick={openAdd}>Post first announcement</Button>}
        </div>
      ) : (
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        ...
                        {a.pinned && (
                          <svg className="h-4 w-4 text-indigo-500 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M16 2L12 6 8 2 4 6l4 4-2 8h12l-2-8 4-4-4-4z" />
                          </svg>
                        )}
                        <h3 className={`font-semibold text-sm ${isRead ? 'text-gray-700 dark:text-slate-200' : 'text-gray-900 dark:text-white'}`}>
                          {a.title}
                        </h3>
                        <Badge variant={catVariant} className="capitalize">{cat}</Badge>
                      ...
                      {isExpanded ? (
                        <p className="text-sm text-gray-600 dark:text-slate-300 whitespace-pre-wrap mt-2">{a.content}</p>
                      ) : (
                        <p className="text-sm text-gray-500 dark:text-slate-400 line-clamp-1">{a.content}</p>
                      )}

                      <div className="mt-2 flex items-center gap-3">
                        <p className="text-xs text-gray-400 dark:text-slate-500">{timeAgo(a.createdAt)}</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input {...register('audience')} className="input w-full" placeholder="All / B.Tech 3rd Year…" />
            </div>
          </div>
